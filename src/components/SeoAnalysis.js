...
import { AnalysisWorkerWrapper, createWorker, Paper, string, helpers } from 'yoastseo'
...
const worker = new AnalysisWorkerWrapper(createWorker(require('./worker?webworker')))
...
function SeoAnalysisImpl({ document: { displayed: document }, mainContentSelector, canonicalUrl, assessmentUrl }) {
  useX({ assessmentUrl, canonicalUrl, mainContentSelector, document })
...
function useX({ assessmentUrl, canonicalUrl, mainContentSelector, document }) {
  React.useEffect(
    () => {
      run()
      ...
      async function run() {
        ...
        const paper = await getPaper({ locale, assessmentUrl, canonicalUrl, mainContentSelector, document })
        const results = await worker.analyze(paper)

        console.log(results)
      }
    },
    [assessmentUrl, canonicalUrl, mainContentSelector, document]
  )
}

async function getPaper({ locale, assessmentUrl, canonicalUrl, mainContentSelector, document }) {
  ...
  const { keyphrase = '', synonyms = '', cornerstone = false } = document.seo ?? {}

  return new Paper(relevantHtml, {
    keyword: keyphrase,
    synonyms,
    description,
    url: canonicalUrl,
